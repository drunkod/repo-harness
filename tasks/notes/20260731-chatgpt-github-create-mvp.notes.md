...
Operator-reported local verification on 2026-08-04 completed successfully:
...
bun test \
  ...
  tests/cli/chatgpt-browser-create-readback.test.ts \
  tests/skill-surface/chatgpt-create-mode.test.ts
```
...
This closes the three verification gaps identified during static review:

1. `BrowserConsultInput.chatgptApp?: never` compiles with Create/read-back
   overriding the spread input to `chatgptApp: undefined` before calling the
   shared transport.
2. `create-mode.ts` contains no dangling
   `assertOracleAppPreselectCapability` reference and never forwards an app
   string into `runBrowserConsult`.
3. Browser Doctor serializes no `oracle.optionalCapabilities` field and the
   regression test confirms app-preselection metadata is absent.

The command output was supplied by the operator; it was not generated by the
GitHub API review environment.
