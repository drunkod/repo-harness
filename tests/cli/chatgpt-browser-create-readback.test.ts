...
function writeFakeOracle(dir: string, output: string): string {
  ...
  writeFileSync(path, [
    ...
    "const id = 'readback-' + crypto.randomUUID();",
    "await Bun.write(process.env.ORACLE_HOME_DIR + '/sessions/' + id + '/meta.json', JSON.stringify({ id }));",
    "await Bun.write(arg('--write-session'), JSON.stringify({ protocol: 1, kind: 'oracle-session', sessionId: id, parentSessionId: arg('--followup') ?? null }));",
