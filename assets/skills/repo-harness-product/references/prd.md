...
3. Activate `$geju` for a direction pass before PRD drafting. Produce a compact geju framing with Thesis, High-geju Direction, Bold Takes, What Not To Do, First Proof Point, and Falsifier. This framing is input to the PRD, not a replacement for the PRD.
...
9. Write a new `plans/prds/<YYYYMMDD>-<HHMM>-<slug>.prd.md`. Fill every core section; include optional sections only when tier or user request requires them, or when the prior-art trigger rule (step 4) makes `## Adjacent Patterns` required. Read `.ai/harness/policy.json#documentation.language` before writing prose: section headings, field keys, and technical terms stay English, and body prose follows that language, where `follow-user` means the language the user is currently writing in.
