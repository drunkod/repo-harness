...
distinct mode from `references/consult.md`: consult remains non-mutating
...
actual repository read/write tools. If the selected app does not expose the
required write actions for the target repository, fail closed.
...
2. Dry-run the exact Create bundle first. Attach only approved workflow
...
3. Inspect the saved `prompt.md`, file manifest, app preselection, and secret
   scan receipt. Do not add unscanned context after the dry run.
4. Run the real Create session with the same bundle and a timestamped,
...
5. In ChatGPT, resolve the repository and base commit through GitHub reads
   before requesting a write. Read `AGENTS.md`, the approved plan, the task
   contract, and every existing file that may be changed.
6. Return a pre-write summary containing the exact repository, base commit,
   target branch, and proposed file list. If any item differs from the
   contract, stop.
7. Create the dedicated branch from the exact recorded base commit. Never
   write directly to the default branch.
8. For an existing file, refetch its current blob/content SHA immediately
   before updating it. A stale-SHA conflict requires a fresh read and a new
   decision; never force the previous replacement.
9. Use the narrowest write path:
...
13. Trust the Browser Engine managed `--write-output` and session metadata as
    the transport record. GitHub commit/PR identifiers are the repository
    record. Neither alone is acceptance.
14. Hand the frozen plan, contract, commit/PR patch, changed-file list, and CI
...
- `ORACLE_APP_PRESELECT_UNSUPPORTED`: the Oracle binary cannot select the
  GitHub app. Upgrade/fix Oracle or select the app manually; do not pretend
...
- `ORACLE_CAPTURE_INCOMPLETE`: the GitHub writes may already have occurred.
  ...
  session through `references/continue.md`; do not submit the Create request
  again from zero.
