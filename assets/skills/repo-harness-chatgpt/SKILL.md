...
description: Canonical rule owner for repo-harness ChatGPT integration -- Oracle-first browser/GPT Pro consult and continuation, GitHub-app-backed Create, MCP Connector setup, MCP bridge planning handoff, and Connector invocation read-back evidence.
when_to_use: "repo-harness-chatgpt, ChatGPT Web consult, GPT Pro consult, gptpro, browser GPT, ChatGPT GitHub Create, GitHub app Create, ChatGPT MCP Connector, ChatGPT bridge, MCP read-back, GPT Pro delegate, delegate to ChatGPT, 外包给 GPT"
---
...
- Consult stays planning/review/critique only. Create is the sole ChatGPT Web mode allowed to perform GitHub writes, and only on a dedicated branch with explicit scope; delegate remains the no-write patch-text path.
